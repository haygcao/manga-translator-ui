...
2. **运行安装**：
   - 双击 `步骤1-首次安装.bat`
   - 脚本会自动：
     - ✓ 检测并安装 Miniconda（轻量级Python环境管理）到项目目录
     - ✓ 下载便携版 Git（可选）
     - ✓ 克隆代码仓库
     - ✓ 在项目目录创建独立的Python环境（conda_env）
...
> 💡 **新特性**：无需预装 Python！脚本会自动安装 Miniconda 到项目目录
> 
> Miniconda 特点：
> - 体积小（安装程序约50MB，安装后约600MB）
> - 可管理多个Python版本（Python 3.12）
> - 环境隔离，不影响系统
> - 自带pip包管理
> - **完全安装在项目目录，不占用C盘系统空间**

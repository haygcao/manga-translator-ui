...
- **高质量翻译批量大小**：高质量翻译时的批量大小（一次发送的文本块数量）
...
- **使用 YOLO OBB**：使用 YOLO 有向边界框检测

- **YOLO OBB 置信度**：YOLO OBB 检测置信度阈值

- **YOLO OBB IOU**：YOLO OBB IOU 阈值

- **YOLO OBB 重叠阈值**：YOLO OBB 框重叠阈值
...
- **优化断行**：启用 AI 断句（需要 OpenAI/Gemini 翻译器）

- **严格智能缩放**：严格智能缩放模式（更严格地控制字体大小）
